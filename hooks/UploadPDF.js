    ...
    if (selectedFile) {
      ...
      fileRef.put(selectedFile)
        .then((snapshot) => {
          ...
          const newFilePath = projectStorage.ref().child(`/${company}/${selectedFile.name}`)
          const newFileName = `Insurance`;
          newFilePath.updateMetadata({customMetadata:{title: newFileName} }).then(function(metadata){
